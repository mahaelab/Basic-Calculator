...
public class Calc extends JFrame {
	...
	public static void main(String[] args) {
		...
		public void addPane(Container pane) {
			...
			//Sets size of field and adds to panel
			...
			panel.add(txt1);panel.add(tf1);panel.add(txt2);panel.add(tf2);panel.add(blank);
			...
			//Answer field is added
			JLabel resultField = new JLabel("Result: ");
			panel.add(resultField);
	
		
		//Action listener defines what action should happen when performing operation
		calculateButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				JOptionPane.showMessageDialog(panel, "Please Enter Two Numbers");
						...
						try {
						double first = Double.parseDouble(tf1.getText().toString());
						double second =Double.parseDouble(tf2.getText().toString());
					
						//Choose an operation
						String op = operation.getSelectedItem().toString();
						double answer = 0;
						//Switch case used since we have multiple operations
							switch(op) {
								case "Add":
									answer = first + second;
									break;
